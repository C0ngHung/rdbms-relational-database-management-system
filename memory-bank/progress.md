...
- [x] **Tài liệu & Bài học nền tảng**:
  ...
  - [Part 2: Chuẩn Hóa Cơ Sở Dữ Liệu - Database Normalization](file:///d:/learnByMyself/course/database/rdbms-relational-database-management-system/sql-server/Part%202%20-%20Standardization/Part%202%20-%20Standardization.md): Hoàn thành 835 dòng chuyên sâu kèm 12 hình ảnh sơ đồ minh họa trực quan (CSDL Good, 3 Anomalies, Principles 1-4, BCNF 3.5, Examples 1-2, Normalization flow) cùng 12 chú thích giải nghĩa sư phạm bằng tiếng Anh (`*Figure 1*` → `*Figure 12*`), phân tích chuyên sâu Phụ thuộc bắc cầu (Lookup Table vs Dynamic CASE WHEN), 4NF Multi-Valued Dependency và hiện tượng bùng nổ tích Descartes, cùng Section §7 Hands-on Lab (bài giải mẫu `RawEnrollments` và bài tập thực hành `LibraryRaw`).
  - [Part 3: Sơ đồ thực thể - quan hệ ERD](file:///d:/learnByMyself/course/database/rdbms-relational-database-management-system/sql-server/Part%203%20-%20Entity-Relationship%20Diagram%20%28ERD%29/Part%203%20-%20Entity-Relationship%20Diagram%20%28ERD%29.md): Hoàn thành bài giảng về ER Model, các thành phần chính (Entity, Attribute, Relationship), ký pháp Chen, và áp dụng thực tế vào bài toán Quản lý thư viện đại học. Bổ sung 12 hình ảnh minh họa chất lượng cao kèm chú thích tiếng Anh (`*Figure 1*` → `*Figure 12*`) giúp minh họa trực quan các khái niệm lý thuyết.
...
| **RDBMS-003**   | Part 3: Sơ đồ thực thể - quan hệ ERD                                              | ✅ Hoàn thành                |
| **RDBMS-004**   | Ràng buộc toàn vẹn & Kiểu dữ liệu chuyên sâu (PK, FK, CHECK, UNIQUE, DOMAIN)      | 📋 Lên kế hoạch              |
